...
## Current next step — Experiment 48

Candidate J is frozen unchanged after Experiment 47. Do **not** tune it on the known seven-day/recent24h evidence. Experiment 48 is a prospective MT5-only validation and is intentionally waiting for a genuinely fresh 48-hour XAUUSD snapshot after the weekend reopen. Use `tools/capture_recent_xau_ticks.py --hours 48 --used-by docs/experiments/2026-09-26/48-v4-5-candidate-j-fresh-validation.md`, then run `research/v4_5_candidate_j_fresh_validation.py` against that immutable snapshot/manifest. The predeclared pass gate requires positive whole-window J P&L, J >= D, positive non-overlapping 4h mean, >=50% positive 4h blocks, and nonnegative +$0.10/side stress on **both** 500 ms and 1 s. Final20 remains sealed.

Experiment 47R stress-tested frozen Candidate J with three new random seeds, 40 real four-hour windows per grid per seed, keeping the same 50/50 historical-first80/recent24h balance. Candidate J 500 ms mean was positive in **all three** batches (+₹0.97, +₹8.13, +₹3.92) and pooled +₹4.34 across 120 windows; D pooled -₹0.85, H -₹4.17. J beat D in 92.5% of pooled 500 ms windows. At 1 s J was +₹0.54/-₹2.85/+₹1.83 and pooled -₹0.16; it beat D in 87.5% of pooled windows but is not confirmed profitable. Median P&L remains negative on both grids, so expectancy is carried by fewer larger winners. Candidate J remains frozen unchanged for Experiment 48 fresh validation; do not tune it on this audit. Final20 remains sealed.

## Candidate J live-shadow harness ready

Experiment 48P is complete. `scripts/live_shadow_candidate_j.py` is a read-only MT5 XAUUSD capture/shadow runner for frozen Candidate D/H/J on 500 ms and 1 s. Offline preflight exactly reproduced Experiment-47 recent24h P&L/trade/win metrics for all six candidate/grid checks. AST safety scan found zero broker execution calls. The runner writes local `data/live_shadow/<session-id>/ticks.csv`, `session_manifest.json`, `shadow_status.json`, and `shadow_events.jsonl`; that folder is gitignored. It reports `MARKET_IDLE` and does not manufacture ticks when the market is closed. Start the actual shadow only after XAUUSD resumes producing fresh ticks. Windows launcher: `scripts/start_candidate_j_shadow.ps1`; `-ConnectCheck` performs a read-only terminal check. Rolling shadow metrics are monitoring only, not Experiment-48 prospective evidence. Experiment 48 still uses a separately frozen fresh 48-hour snapshot and the predeclared pass gate. Final20 remains sealed.
