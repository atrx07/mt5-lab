...
Experiment 32 freezes Router v3 before measuring its P&L. It keeps Candidate D's PRIMARY → SECONDARY → MICRO → BURST priority and keeps the Router-v2 score unchanged, but converts that score from a global ranker into a veto. A vetoed engine is latched for its already-existing Candidate-D cooldown horizon, preventing immediate retries of the same rejected setup; routing then falls through to lower-priority eligible engines. This adds no fitted threshold or cooldown. Known-data results can reject the architecture, but cannot promote it; a future unseen snapshot remains mandatory.
