...
There are no runs yet. Use `backtests/` for historical replay, `forward/` for timestamped shadow or paper observations, and `benchmarks/` for latency, cost, data, and execution diagnostics. Each experiment gets one directory with the same ID as its document and contains at least `README.md`, `config.json`, and `summary.json`.
