...
`shadow.py` constructs timestamp-causal numeric state for a baseline entry candidate. The optional candidate hook in `paper.py` exposes only bars through the signal candle, leaving the baseline policy and accounting unchanged.
